...
train_dataset = MoveObjs('./data/MoveObjs/raw/','train',4000,1)
val_dataset = MoveObjs('./data/MoveObjs/raw/','test',500,1)
...
def eval_model(model,dataloader_val,writer,iter_idx):
    ...
    for data_eval in dataloader_val:
        ...
        # Forward propagation
        loss_dict,out_dict = model(pcd_video[:,0],rgb_video[:,0],depth_video[:,0])

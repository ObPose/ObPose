...
from model.obpose_icsbp import ObPose
...
ex_id = args.ex_id
it_num = args.it
test_dataset = ObjTabelMultiview('./data/obj_tabel_multiview/raw/','test',1500)
dataloader_test = DataLoader(test_dataset,batch_size=1,shuffle=False)
directory_output = './checkpoints/obj_tabel_multiview/{}/'.format(args.ex_id)
...
for data in dataloader_test:
    ...
    with torch.no_grad():
        loss_dict,out_dict = model(pcd,rgb,depth,camera_in,camera_ex)
    for t in [0]:
...
torch.save([iou_bg_mean,ari_obj_mean,msc_obj_mean],f'./eval_results/seg/{ex_id}-{it_num}_icsbp.pt')
